...
namespace Tavenem.Mathematics.Decimals
{
    ...
    public class Cuboid : IShape, IEquatable<Cuboid>, ISerializable
    {
        ...
        public Cuboid(decimal axisX, decimal axisY, decimal axisZ)
        ...
        public Cuboid(decimal axisX, decimal axisY, decimal axisZ, Vector3 position)
        ...
        public Cuboid(decimal axisX, decimal axisY, decimal axisZ, Vector3 position, Quaternion rotation)
        {
            AxisX = axisX;
            AxisY = axisY;
            AxisZ = axisZ;
            ...
            ContainingRadius = ((AxisX * AxisX) + (AxisY * AxisY) + (AxisZ * AxisZ)).Sqrt() / 2;
        ...
        private bool Intersects(Cone cone)
        {
            ...
            var cosAngle = (decimal)Math.Cos(Math.Atan((double)(cone.Radius / cone.Length)));
            var cosSqr = cosAngle * cosAngle;
            Vector3[] X = new Vector3[8], PmV = new Vector3[8];
            decimal[] DdPmV = new decimal[8], sqrDdPmV = new decimal[8], sqrLenPmV = new decimal[8];
            int iMax = -1, jMax = -1;
            ...
            var DdUA = new decimal[3]
            {
                DdU.X,
                DdU.Y,
                DdU.Z,
            };
            var maxPmVA = new decimal[3]
            {
                PmV[jMax].X,
                PmV[jMax].Y,
                PmV[jMax].Z,
            ...
            if (fder > 0)
            {
                ...
                var MmVA = new decimal[3] { 0, 0, 0, };
                var maxXA = new decimal[3]
                {
                    X[jMax].X,
                    X[jMax].Y,
                    X[jMax].Z,
                };
                var CmVA = new decimal[3]
                {
                    CmV.X,
                    CmV.Y,
                    CmV.Z,
