...
namespace Tavenem.Mathematics.Decimals
{
    ...
    public class HollowSphere : IShape, IEquatable<HollowSphere>, ISerializable
    {
        ...
        public decimal ContainingRadius => OuterRadius;
        ...
        /// </summary>
        [DataMember(Order = 1)]
        public decimal InnerRadius { get; }
        ...
        /// </summary>
        [DataMember(Order = 2)]
        public decimal OuterRadius { get; }
        ...
        /// </remarks>
        [System.Text.Json.Serialization.JsonIgnore]
        public decimal SmallestDimension => OuterRadius;
        ...
        public HollowSphere(decimal innerRadius, decimal outerRadius)
        ...
        public HollowSphere(decimal innerRadius, decimal outerRadius, Vector3 position)
        {
            ...
            Volume = (DecimalConstants.FourThirdsPI * (decimal)Math.Pow((double)OuterRadius, 3)) - (DecimalConstants.FourThirdsPI * (decimal)Math.Pow((double)InnerRadius, 3));
        }

        private HollowSphere(SerializationInfo info, StreamingContext context) : this(
            (decimal?)info.GetValue(nameof(InnerRadius), typeof(decimal)) ?? default,
            (decimal?)info.GetValue(nameof(OuterRadius), typeof(decimal)) ?? default,
        ...
        private bool Intersects(Line line)
        {
            ...
            else
            {
                ...
                var qm = tmp0 - tmp1;
                var qp = tmp0 + tmp1;
                var intersect = qm * qp <= 0 || (qm > 0 && Math.Abs(a1) < extent);
                if (!intersect)
                {
                    return false;
                }
