...
namespace Tavenem.Mathematics.Decimals
{
    ...
    public class Line : IShape, IEquatable<Line>, ISerializable
    {
        ...
        private static void ComputeIntersection(decimal mF00, decimal mF10, decimal mB, decimal[] sValue, int[] classify, out int[] edge, out decimal[,] end)
        {
            edge = new int[2];
            end = new decimal[2, 2];
        ...
        private static void ComputeMinimumParameters(decimal mG00, decimal mG01, decimal mG10, decimal mG11, decimal mB, decimal mC, decimal mE, int[] edge, decimal[,] end, out decimal[] parameters)
        {
            parameters = new decimal[2];
        ...
        private static decimal GetClampedRoot(decimal slope, decimal h0, decimal h1)
        {
            if (h0 < 0)
            {
                if (h1 > 0)
        ...
        public decimal GetDistanceTo(Vector3 point, out Vector3 closestPoint)
        {
            var diff = point - _end;
            closestPoint = _start;
        ...
        /// this instance to the given <paramref name="point"/>.</param>
        /// <returns>The distance to the given <paramref name="point"/>.</returns>
        public decimal GetDistanceTo(SinglePoint point, out Vector3 closestPoint)
        ...
        public decimal GetDistanceTo(Line line, out Vector3 closestPoint, out Vector3 closestPointOther)
        {
            ...
            var mG01 = mG00 + mC;
            var mG11 = mG10 + mC;
            var parameters = new decimal[2];

            if (mA > 0 && mC > 0)
            {
                var sValue = new decimal[2];
        ...
        public bool Intersects(IShape shape)
        {
            ...
            {
                return GetDistanceTo(point, out _) == 0;
            }
        ...
        public bool IsPointWithin(Vector3 point) => GetDistanceTo(point, out var _) == 0;
        ...
        private bool Intersects(Line line)
        {
            ...
            if (Vector3.Dot(startDiff, crossPaths) != 0)
            {
                return false;
            }
