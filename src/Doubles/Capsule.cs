...
namespace Tavenem.Mathematics.Doubles
{
    ...
    public class Capsule : IShape, IEquatable<Capsule>, ISerializable
    {
        ...
        public Capsule(Vector3 axis, double radius) : this(axis, radius, Vector3.Zero) { }
        ...
        public Capsule(Vector3 axis, double radius, Vector3 position)
        {
            Axis = axis;
            Position = position;
            Radius = radius;
        ...
        private static int FindBoxEdgeIntersection(double ay, double ey, double dx, double dz, double bx, double by, double bz, double radius, out double distance)
        {
            distance = 0;

            var rSqr = radius * radius;
            double dy, crossZ, crossX;
        ...
        private static int FindBoxEdgeRegionIntersection(
            ...
            bool isAboveEdge, double radius, out double distance)
        {
            distance = 0;
        ...
        private static int FindBoxFaceRegionIntersection(
            ...
            double ax, double ay, double az,
            double bx, double by, double bz,
            bool isAboveFace, double radius, out double distance)
        {
            distance = 0;
        ...
        private static int FindBoxVertexRegionIntersection(
            ...
            double ax, double ay, double az,
            double bx, double by, double bz,
            double radius, out double distance)
        {
            distance = 0;
        ...
        private static double GetBoxEdgeIntersection(double dx, double dz, double bx, double bz, double bSqr, double rSqr)
        {
            var dot = (bx * dx) + (bz * dz);
            var diff = (dx * dx) + (dz * dz) - rSqr;
            var inv = 1 / Math.Sqrt(Math.Abs((dot * dot) - (bSqr * diff)));
            return diff * inv / (1 - (dot * inv));
        }

        private static double GetBoxVertexIntersection(
        ...
        private static bool Quadratic(double a, double b, double c, out double root1, out double root2)
        {
            var q = (b * b) - (4 * a * c);
            if (q >= 0)
            {
        ...
        public bool IsPointWithin(Vector3 point)
        {
            ...
            var distance = sqrDist.IsNearlyEqualTo(sqrRadius) ? Math.Sqrt(sqrDist) - Radius : 0;
        ...
        public IShape ScaleVolume(double factor)
        {
            ...
            {
                var hMultiplier = Math.Sqrt(factor);
                return new Capsule(Axis * hMultiplier, Radius * Math.Sqrt(hMultiplier), Position);
            }
        }

        private double? GetDistanceToCollisionPoint(IShape shape)
        {
            if (shape is SinglePoint point)
            {
                return new Line(Axis, Position).GetDistanceTo(point, out var closestPoint).IsNearlyZero()
                    ? (closestPoint - _start).Length()
                    : (double?)null;
            }
            if (shape is Line line)
            {
                return Intersects(line, out var d) ? d : (double?)null;
            }
            if (shape is Sphere sphere)
            {
                return Intersects(sphere.Radius, sphere.Position, sphere.Position, out var d) ? d : (double?)null;
            }
            if (shape is Capsule capsule)
            {
                return Intersects(capsule.Radius, capsule._start, capsule._end, out var d) ? d : (double?)null;
            }
            if (shape is Cuboid cuboid)
            {
                return !Intersects(cuboid, out var d) ? null : (double?)d;
            }
            // Substitute the sphere formed by the containing radius.
            return Intersects(shape.ContainingRadius, shape.Position, shape.Position, out var dist) ? dist : (double?)null;
        }

        private bool Intersects(Cuboid cuboid, out double distance)
        {
            distance = 0;
        ...
        private bool Intersects(Line line, out double distance)
        {
            distance = 0;
            var orthonormalBasis = new Vector3[]
            {
            ...
            double? intersect1 = null, intersect2 = null;
            if (Math.Abs(dz).IsNearlyEqualTo(1))
            {
            ...
            if (discr > 0)
            {
                ...
                if (intersect2.HasValue) // the line intersects with the cylinder in two places
                {
                    ...
                    intersect1 = Math.Sqrt((a * a) - rSqr);
                    var h = line.GetDistanceTo(_start, out _);
                    var totalDistance = Math.Sqrt((a * a) - (h * h));
            ...
            else
            {
                ...
                if (Math.Abs(zValue) <= line.Length) // the line intersects the cylinder at a single point
                {
                    // calculate the distance the starting sphere must travel to encounter the line
                    var a = line.GetDistanceTo(new Line(Axis, Position), out _, out _);
                    var distance1 = Math.Sqrt((a * a) - rSqr);
                    var h = line.GetDistanceTo(_start, out _);
                    var totalDistance = Math.Sqrt((a * a) - (h * h));
        ...
        private bool Intersects(double otherRadius, Vector3 B0, Vector3 B1, out double distance)
        {
            var AB = B0 - _start;
            var vab = B1 - B0 - Axis;
