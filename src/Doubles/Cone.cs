...
namespace Tavenem.Mathematics.Doubles
{
    ...
    public class Cone : IShape, IEquatable<Cone>, ISerializable
    {
        ...
        public Cone(Vector3 axis, double radius) : this(axis, radius, Vector3.Zero) { }
        ...
        public Cone(Vector3 axis, double radius, Vector3 position)
        {
            Axis = axis;
            Position = position;
            Radius = radius;
            ...
            ContainingRadius = Math.Sqrt((halfHeight * halfHeight) + (Radius * Radius));
        ...
        public Cone(Vector3 origin, Vector3 orientation, double angle)
        {
            Axis = orientation;
        ...
        public bool IsPointWithin(Vector3 point)
        {
            ...
            return dot / p1.Length() / Length > Math.Cos(Math.Atan(Radius / Length) / 2)
                && dot / Length < Length;
        }
        ...
        public IShape ScaleVolume(double factor)
        {
            ...
            else
            {
                ...
                return new Cone(Axis * hMultiplier, Radius * Math.Sqrt(hMultiplier));
            }
        }

        internal double GetRadiusAtPoint(Vector3 point) => Radius / Length * (point - Position).Length();
        ...
        private bool Intersects(Line line)
        {
            ...
            var cosAngle = Math.Cos(Math.Atan(Radius / Length));
            ...
            if (!c2.IsNearlyZero())
            {
                ...
                if (discr > 0)
                {
                    ...
                    else if (numIntersects == 1)
                    {
                        if (DdU > 0)
                        {
                            intersects[1] = double.PositiveInfinity;
                        }
                        else
                        {
                            intersects[1] = intersects[0];
                            intersects[0] = double.PositiveInfinity;
                        }
                    }
            ...
            else if (!c1.IsNearlyZero())
            {
                ...
                if ((DdU * t) + DdPmV >= 0)
                {
                    ...
                    {
                        intersects[0] = double.NegativeInfinity;
                        intersects[1] = t;
                    }
                }
                else
        ...
        {
            var angle = Math.Atan(Radius / Length);
