...
namespace Tavenem.Mathematics.Doubles
{
    ...
    public class Cuboid : IShape, IEquatable<Cuboid>, ISerializable
    {
        ...
        public double ContainingRadius { get; }
        ...
        /// </remarks>
        [System.Text.Json.Serialization.JsonIgnore]
        public double SmallestDimension { get; }
        ...
        public Cuboid(double axisX, double axisY, double axisZ)
        ...
        public Cuboid(double axisX, double axisY, double axisZ, Vector3 position)
        ...
        public Cuboid(double axisX, double axisY, double axisZ, Vector3 position, Quaternion rotation)
        {
            AxisX = axisX;
            AxisY = axisY;
            AxisZ = axisZ;
            ...
            ContainingRadius = Math.Sqrt((AxisX * AxisX) + (AxisY * AxisY) + (AxisZ * AxisZ)) / 2;
        ...
        private Cuboid(SerializationInfo info, StreamingContext context) : this(
            (double?)info.GetValue(nameof(AxisX), typeof(double)) ?? default,
            (double?)info.GetValue(nameof(AxisY), typeof(double)) ?? default,
            (double?)info.GetValue(nameof(AxisZ), typeof(double)) ?? default,
        ...
        public IShape ScaleVolume(double factor)
        {
            ...
            {
                var multiplier = Math.Pow(factor, 1.0 / 3.0);
        ...
        private bool Intersects(Cone cone)
        {
            ...
            var cosAngle = Math.Cos(Math.Atan(cone.Radius / cone.Length));
            var cosSqr = cosAngle * cosAngle;
            Vector3[] X = new Vector3[8], PmV = new Vector3[8];
            double[] DdPmV = new double[8], sqrDdPmV = new double[8], sqrLenPmV = new double[8];
            int iMax = -1, jMax = -1;
            ...
            var DdUA = new double[3]
            {
                DdU.X,
                DdU.Y,
                DdU.Z,
            };
            var maxPmVA = new double[3]
            {
                PmV[jMax].X,
                PmV[jMax].Y,
                PmV[jMax].Z,
            ...
            if (fder > 0)
            {
                ...
                var MmVA = new double[3] { 0, 0, 0, };
                var maxXA = new double[3]
                {
                    X[jMax].X,
                    X[jMax].Y,
                    X[jMax].Z,
                };
                var CmVA = new double[3]
                {
                    CmV.X,
                    CmV.Y,
                    CmV.Z,
        ...
        private bool Intersects(Cuboid cuboid)
        {
            ...
            for (var i = 0; i < 3; i++)
            {
                ...
                rb = (cuboid.AxisX * Math.Abs(R[i, 0]))
                    + (cuboid.AxisY * Math.Abs(R[i, 1]))
                    + (cuboid.AxisZ * Math.Abs(R[i, 2]));
                if (i == 0)
                {
                    t = Math.Abs(T.X);
                }
            ...
            for (var k = 0; k < 3; k++)
            {
                ...
                ra = (AxisX * Math.Abs(R[0, k]))
                    + (AxisY * Math.Abs(R[1, k]))
                    + (AxisZ * Math.Abs(R[2, k]));
                t = Math.Abs((T.X * R[0, k]) + (T.Y * R[1, k]) + (T.Z * R[2, k]));
                if (t > ra + rb)
                {
                    return false;
                }
            ...
            // L = A0 x B0
            ra = (AxisY * Math.Abs(R[2, 0])) + (AxisZ * Math.Abs(R[1, 0]));
            rb = (cuboid.AxisY * Math.Abs(R[0, 2])) + (AxisZ * Math.Abs(R[0, 1]));
            t = Math.Abs((T.Z * R[1, 0]) - (T.Y * R[2, 0]));
            if (t > ra + rb)
            {
                return false;
            }
            ...
            // L = A0 x B1
            ra = (AxisY * Math.Abs(R[2, 1])) + (AxisZ * Math.Abs(R[1, 1]));
            rb = (cuboid.AxisX * Math.Abs(R[0, 2])) + (AxisZ * Math.Abs(R[0, 0]));
            t = Math.Abs((T.Z * R[1, 1]) - (T.Y * R[2, 1]));
            if (t > ra + rb)
            {
                return false;
            }
            ...
            // L = A0 x B2
            ra = (AxisY * Math.Abs(R[2, 2])) + (AxisZ * Math.Abs(R[1, 2]));
            rb = (cuboid.AxisX * Math.Abs(R[0, 1])) + (AxisY * Math.Abs(R[0, 0]));
            t = Math.Abs((T.Z * R[1, 2]) - (T.Y * R[2, 2]));
            if (t > ra + rb)
            {
                return false;
            }
            ...
            // L = A1 x B0
            ra = (AxisX * Math.Abs(R[2, 0])) + (AxisZ * Math.Abs(R[0, 0]));
            rb = (cuboid.AxisY * Math.Abs(R[1, 2])) + (AxisZ * Math.Abs(R[1, 1]));
            t = Math.Abs((T.X * R[2, 0]) - (T.Z * R[0, 0]));
            if (t > ra + rb)
            {
                return false;
            }
            ...
            // L = A1 x B1
            ra = (AxisX * Math.Abs(R[2, 1])) + (AxisZ * Math.Abs(R[0, 1]));
            rb = (cuboid.AxisX * Math.Abs(R[1, 2])) + (AxisZ * Math.Abs(R[1, 0]));
            t = Math.Abs((T.X * R[2, 1]) - (T.Z * R[0, 1]));
            if (t > ra + rb)
            {
                return false;
            }
            ...
            // L = A1 x B2
            ra = (AxisX * Math.Abs(R[2, 2])) + (AxisZ * Math.Abs(R[0, 2]));
            rb = (cuboid.AxisX * Math.Abs(R[1, 1])) + (AxisY * Math.Abs(R[1, 0]));
            t = Math.Abs((T.X * R[2, 2]) - (T.Z * R[0, 2]));
            if (t > ra + rb)
            {
                return false;
            }
            ...
            // L = A2 x B0
            ra = (AxisX * Math.Abs(R[1, 0])) + (AxisY * Math.Abs(R[0, 0]));
            rb = (cuboid.AxisY * Math.Abs(R[2, 2])) + (AxisZ * Math.Abs(R[2, 1]));
            t = Math.Abs((T.Y * R[0, 0]) - (T.X * R[1, 0]));
            if (t > ra + rb)
            {
                return false;
            }
            ...
            // L = A2 x B1
            ra = (AxisX * Math.Abs(R[1, 1])) + (AxisY * Math.Abs(R[0, 1]));
            rb = (cuboid.AxisX * Math.Abs(R[2, 2])) + (AxisZ * Math.Abs(R[2, 0]));
            t = Math.Abs((T.Y * R[0, 1]) - (T.X * R[1, 1]));
            if (t > ra + rb)
            {
                return false;
            }
            ...
            // L = A2 x B2
            ra = (AxisX * Math.Abs(R[1, 2])) + (AxisY * Math.Abs(R[0, 2]));
            rb = (cuboid.AxisX * Math.Abs(R[2, 1])) + (AxisY * Math.Abs(R[2, 0]));
            t = Math.Abs((T.Y * R[0, 2]) - (T.X * R[1, 2]));
        ...
        private bool Intersects(Line line)
        {
            ...
            if (Math.Abs(offsetLineCenter.X) > AxisX + (line.ContainingRadius * Math.Abs(offsetLineDirection.X)))
            {
                return false;
            }
            if (Math.Abs(offsetLineCenter.Y) > AxisY + (line.ContainingRadius * Math.Abs(offsetLineDirection.Y)))
            {
                return false;
            }
            if (Math.Abs(offsetLineCenter.Z) > AxisZ + (line.ContainingRadius * Math.Abs(offsetLineDirection.Z)))
            {
                return false;
            }

            var WxD = Vector3.Cross(offsetLineDirection, offsetLineCenter);
            var absWdU = new double[]
            {
                Math.Abs(offsetLineDirection.X),
                Math.Abs(offsetLineDirection.Y),
                Math.Abs(offsetLineDirection.Z)
            };
            return (Math.Abs(WxD.X) <= (AxisY * absWdU[2]) + (AxisZ * absWdU[1]))
                && (Math.Abs(WxD.Y) <= (AxisX * absWdU[2]) + (AxisZ * absWdU[0]))
                && (Math.Abs(WxD.Z) <= (AxisX * absWdU[1]) + (AxisY * absWdU[0]));
        }
