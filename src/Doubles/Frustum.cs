...
namespace Tavenem.Mathematics.Doubles
{
    ...
    public class Frustum : IShape, IEquatable<Frustum>, ISerializable
    {
        ...
        /// </summary>
        [DataMember(Order = 3)]
        public double FieldOfViewAngle { get; }
        ...
        /// clipping plane occurs.
        /// </param>
        public Frustum(double aspectRatio, Vector3 axis, double fieldOfViewAngle, double nearPlaneDistance)
        ...
        public Frustum(double aspectRatio, Vector3 axis, double fieldOfViewAngle, double nearPlaneDistance, Vector3 position)
        ...
        public Frustum(double aspectRatio, Vector3 axis, double fieldOfViewAngle, double nearPlaneDistance, Vector3 position, Quaternion rotation)
        {
            ...
            FarPlaneDistance = Math.Sqrt(farPlaneDistSq);

            var tan = Math.Tan(fieldOfViewAngle);
            ...
            ContainingRadius = Math.Sqrt((FarPlaneDistance - nearPlaneDistance).Square() + (tanSq4 * farPlaneDistSq * (1 + aspectRatio.Square()))) / 2;
        ...
        private Frustum(SerializationInfo info, StreamingContext context) : this(
            (double?)info.GetValue(nameof(AspectRatio), typeof(double)) ?? default,
            (Vector3?)info.GetValue(nameof(Axis), typeof(Vector3)) ?? default,
            (double?)info.GetValue(nameof(FieldOfViewAngle), typeof(double)) ?? default,
            (double?)info.GetValue(nameof(NearPlaneDistance), typeof(double)) ?? default,
        ...
        private static double GetDistance(Frustum frustum, Vector3 cornerA, Vector3 cornerB, Vector3 cornerC, Vector3 cornerD)
        {
        ...
        private static double GetEdgeDistance(Frustum frustum, Cuboid cuboid)
        {
        ...
        private static double GetEdgeDistance(Frustum frustumA, Frustum frustumB)
        {
            var distance = double.PositiveInfinity;
        ...
        private static double GetFaceDistance(Plane[] planes, Vector3[] corners)
        {
        ...
        private double GetPointDistance(Vector3 point)
        {
