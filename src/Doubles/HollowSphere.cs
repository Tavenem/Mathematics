...
namespace Tavenem.Mathematics.Doubles
{
    ...
    public class HollowSphere : IShape, IEquatable<HollowSphere>, ISerializable
    {
        ...
        public double ContainingRadius => OuterRadius;
        ...
        /// </remarks>
        [System.Text.Json.Serialization.JsonIgnore]
        public double SmallestDimension => OuterRadius;
        ...
        public HollowSphere(double innerRadius, double outerRadius, Vector3 position)
        {
            ...
            Volume = (DoubleConstants.FourThirdsPI * Math.Pow(OuterRadius, 3)) - (DoubleConstants.FourThirdsPI * Math.Pow(InnerRadius, 3));
        }

        private HollowSphere(SerializationInfo info, StreamingContext context) : this(
            (double?)info.GetValue(nameof(InnerRadius), typeof(double)) ?? default,
            (double?)info.GetValue(nameof(OuterRadius), typeof(double)) ?? default,
