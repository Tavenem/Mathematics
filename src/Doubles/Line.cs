...
namespace Tavenem.Mathematics.Doubles
{
    ...
    public class Line : IShape, IEquatable<Line>, ISerializable
    {
        ...
        /// </remarks>
        [System.Text.Json.Serialization.JsonIgnore]
        public double SmallestDimension => 0;
        ...
        private static void ComputeIntersection(double mF00, double mF10, double mB, double[] sValue, int[] classify, out int[] edge, out double[,] end)
        {
            edge = new int[2];
            end = new double[2, 2];
        ...
        private static void ComputeMinimumParameters(double mG00, double mG01, double mG10, double mG11, double mB, double mC, double mE, int[] edge, double[,] end, out double[] parameters)
        {
        ...
        private static double GetClampedRoot(double slope, double h0, double h1)
        {
            if (h0 < 0)
            {
                if (h1 > 0)
        ...
        public double GetDistanceTo(Vector3 point, out Vector3 closestPoint)
        {
            var diff = point - _end;
            closestPoint = _start;
        ...
        /// this instance to the given <paramref name="point"/>.</param>
        /// <returns>The distance to the given <paramref name="point"/>.</returns>
        public double GetDistanceTo(SinglePoint point, out Vector3 closestPoint)
        ...
        public double GetDistanceTo(Line line, out Vector3 closestPoint, out Vector3 closestPointOther)
        {
        ...
        public bool IsPointWithin(Vector3 point) => GetDistanceTo(point, out var _).IsNearlyZero();
        ...
        /// <paramref name="factor"/> must be &gt;= 0.
        /// </exception>
        public IShape ScaleVolume(double factor) => this;
        ...
        private bool Intersects(Line line)
        {
            ...
            if (!Vector3.Dot(startDiff, crossPaths).IsNearlyZero())
            {
                return false;
            }
