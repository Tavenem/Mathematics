...
namespace Tavenem.Mathematics.Doubles
{
    ...
    public class Torus : IShape, IEquatable<Torus>, ISerializable
    {
        ...
        public Torus(double majorRadius, double minorRadius, Vector3 position)
        ...
        public Torus(double majorRadius, double minorRadius, Vector3 position, Quaternion rotation)
        {
            ...
            Volume = DoubleConstants.TwoPISquared * MajorRadius * Math.Pow(MinorRadius, 2);
        }

        private Torus(SerializationInfo info, StreamingContext context) : this(
            (double?)info.GetValue(nameof(MajorRadius), typeof(double)) ?? default,
            (double?)info.GetValue(nameof(MinorRadius), typeof(double)) ?? default,
        ...
        public IShape ScaleVolume(double factor)
        {
            ...
            {
                var majMultiplier = Math.Sqrt(factor);
                return new Torus(MajorRadius * majMultiplier, MinorRadius * Math.Sqrt(majMultiplier), Position, Rotation);
            }
        }
