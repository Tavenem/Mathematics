...
namespace Tavenem.Mathematics.HugeNumbers
{
    ...
    public class Capsule : IShape, IEquatable<Capsule>, ISerializable
    {
        ...
        private readonly HugeNumber _pathLength;
        ...
        /// </summary>
        public Capsule() : this(Vector3.Zero, HugeNumber.Zero) { }
        ...
        public Capsule(Vector3 axis, HugeNumber radius) : this(axis, radius, Vector3.Zero) { }
        ...
        public Capsule(Vector3 axis, HugeNumber radius, Vector3 position)
        {
            Axis = axis;
            Position = position;
            Radius = radius;
        ...
        private static int FindBoxEdgeIntersection(HugeNumber ay, HugeNumber ey, HugeNumber dx, HugeNumber dz, HugeNumber bx, HugeNumber by, HugeNumber bz, HugeNumber radius, out HugeNumber distance)
        {
            distance = 0;

            var rSqr = radius * radius;
            HugeNumber dy, crossZ, crossX;
        ...
        private static int FindBoxEdgeRegionIntersection(
            HugeNumber ex, HugeNumber ey, HugeNumber ez,
            HugeNumber ax, HugeNumber ay, HugeNumber az,
            HugeNumber bx, HugeNumber by, HugeNumber bz,
            bool isAboveEdge, HugeNumber radius, out HugeNumber distance)
        {
            distance = 0;
        ...
        private static int FindBoxFaceRegionIntersection(
            ...
            HugeNumber ax, HugeNumber ay, HugeNumber az,
            HugeNumber bx, HugeNumber by, HugeNumber bz,
            bool isAboveFace, HugeNumber radius, out HugeNumber distance)
        {
            distance = 0;
        ...
        private static int FindBoxVertexRegionIntersection(
            ...
            HugeNumber ax, HugeNumber ay, HugeNumber az,
            HugeNumber bx, HugeNumber by, HugeNumber bz,
            HugeNumber radius, out HugeNumber distance)
        {
            distance = 0;
        ...
        private static HugeNumber GetBoxEdgeIntersection(HugeNumber dx, HugeNumber dz, HugeNumber bx, HugeNumber bz, HugeNumber bSqr, HugeNumber rSqr)
        {
            var dot = (bx * dx) + (bz * dz);
            var diff = (dx * dx) + (dz * dz) - rSqr;
            var inv = 1 / HugeNumber.Sqrt(HugeNumber.Abs((dot * dot) - (bSqr * diff)));
            return diff * inv / (1 - (dot * inv));
        }

        private static HugeNumber GetBoxVertexIntersection(
            HugeNumber dx, HugeNumber dy, HugeNumber dz,
            HugeNumber bx, HugeNumber by, HugeNumber bz,
            HugeNumber rSqr)
        {
        ...
        private static bool Quadratic(HugeNumber a, HugeNumber b, HugeNumber c, out HugeNumber root1, out HugeNumber root2)
        {
            var q = (b * b) - (4 * a * c);
            if (q >= 0)
            {
                var sq = HugeNumber.Sqrt(q);
                var d = 1 / (2 * a);
                root1 = (-b + sq) * d;
                root2 = (-b - sq) * d;
                return true;
            ...
            {
                root1 = HugeNumber.NaN;
                root2 = HugeNumber.NaN;
                return false;
            }
        }
        ...
        public bool IsPointWithin(Vector3 point)
        {
            ...
            var distance = sqrDist.IsNearlyEqualTo(sqrRadius) ? HugeNumber.Sqrt(sqrDist) - Radius : 0;
        ...
        public IShape ScaleVolume(HugeNumber factor)
        {
            ...
            {
                var hMultiplier = HugeNumber.Sqrt(factor);
                return new Capsule(Axis * hMultiplier, Radius * HugeNumber.Sqrt(hMultiplier), Position);
            }
        }

        private HugeNumber? GetDistanceToCollisionPoint(IShape shape)
        {
            if (shape is SinglePoint point)
            {
                return new Line(Axis, Position).GetDistanceTo(point, out var closestPoint).IsZero
                    ? (closestPoint - _start).Length()
                    : (HugeNumber?)null;
            }
            if (shape is Line line)
            {
                return Intersects(line, out var d) ? d : (HugeNumber?)null;
            }
            if (shape is Sphere sphere)
            {
                return Intersects(sphere.Radius, sphere.Position, sphere.Position, out var d) ? d : (HugeNumber?)null;
            }
            if (shape is Capsule capsule)
            {
                return Intersects(capsule.Radius, capsule._start, capsule._end, out var d) ? d : (HugeNumber?)null;
            }
            if (shape is Cuboid cuboid)
            {
                return !Intersects(cuboid, out var d) ? null : (HugeNumber?)d;
            }
            // Substitute the sphere formed by the containing radius.
            return Intersects(shape.ContainingRadius, shape.Position, shape.Position, out var dist) ? dist : (HugeNumber?)null;
        }

        private bool Intersects(Cuboid cuboid, out HugeNumber distance)
        {
            distance = 0;
        ...
        private bool Intersects(Line line, out HugeNumber distance)
        {
            distance = 0;
            var orthonormalBasis = new Vector3[]
            {
            ...
            HugeNumber? intersect1 = null, intersect2 = null;
            if (HugeNumber.Abs(dz).IsNearlyEqualTo(1))
            {
            ...
            if (discr > 0)
            {
                var root = HugeNumber.Sqrt(discr);
                var inv = 1 / a2;
                var tValue = (-a1 - root) * inv;
                var zValue = P.Z + (tValue * D.Z);
                if (HugeNumber.Abs(zValue) <= line.Length)
                {
                    intersect1 = tValue;
                }
                ...
                if (intersect2.HasValue) // the line intersects with the cylinder in two places
                {
                    ...
                    intersect1 = HugeNumber.Sqrt((a * a) - rSqr);
                    var h = line.GetDistanceTo(_start, out _);
                    var totalDistance = HugeNumber.Sqrt((a * a) - (h * h));
            ...
            else
            {
                var tValue = -a1 / a2;
                var zValue = P.Z + (tValue * D.Z);
                if (HugeNumber.Abs(zValue) <= line.Length) // the line intersects the cylinder at a single point
                {
                    // calculate the distance the starting sphere must travel to encounter the line
                    var a = line.GetDistanceTo(new Line(Axis, Position), out _, out _);
                    var distance1 = HugeNumber.Sqrt((a * a) - rSqr);
                    var h = line.GetDistanceTo(_start, out _);
                    var totalDistance = HugeNumber.Sqrt((a * a) - (h * h));
        ...
        private bool Intersects(HugeNumber otherRadius, Vector3 B0, Vector3 B1, out HugeNumber distance)
        {
            var AB = B0 - _start;
            var vab = B1 - B0 - Axis;
