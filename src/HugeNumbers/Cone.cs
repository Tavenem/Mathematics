...
namespace Tavenem.Mathematics.HugeNumbers
{
    ...
    public class Cone : IShape, IEquatable<Cone>, ISerializable
    {
        ...
        /// </summary>
        public Cone() : this(Vector3.Zero, HugeNumber.Zero) { }
        ...
        public Cone(Vector3 axis, HugeNumber radius) : this(axis, radius, Vector3.Zero) { }
        ...
        public Cone(Vector3 axis, HugeNumber radius, Vector3 position)
        {
            Axis = axis;
            Position = position;
            Radius = radius;
            ...
            ContainingRadius = HugeNumber.Sqrt((halfHeight * halfHeight) + (Radius * Radius));

            var normalAxis = Vector3.Normalize(axis);
            var _halfPath = normalAxis * halfHeight;
            ...
            Volume = HugeNumber.PI * Radius * Radius * (Length / 3);
        }
        ...
        public Cone(Vector3 origin, Vector3 orientation, HugeNumber angle)
        {
            Axis = orientation;
            ...
            Radius = HugeNumber.Tan(angle / 2) * Length;
            ContainingRadius = HugeNumber.Sqrt((halfHeight * halfHeight) + (Radius * Radius));
        ...
        public bool IsPointWithin(Vector3 point)
        {
            ...
            return dot / p1.Length() / Length > HugeNumber.Cos(HugeNumber.Atan(Radius / Length) / 2)
                && dot / Length < Length;
        }
        ...
        public IShape ScaleVolume(HugeNumber factor)
        {
            ...
            else
            {
                ...
                return new Cone(Axis * hMultiplier, Radius * HugeNumber.Sqrt(hMultiplier));
            }
        }

        internal HugeNumber GetRadiusAtPoint(Vector3 point) => Radius / Length * (point - Position).Length();
        ...
        private bool Intersects(Capsule capsule)
        {
            ...
            var radiusAtClosestPoint = Radius / Length * (closestPoint - Position).Length();
        ...
        private bool Intersects(Line line)
        {
            ...
            var cosAngle = HugeNumber.Cos(HugeNumber.Atan(Radius / Length));
            ...
            HugeNumber t;
            var intersects = new HugeNumber[2];
            ...
            if (!c2.IsZero)
            {
                ...
                if (discr > 0)
                {
                    ...
                    else if (numIntersects == 1)
                    {
                        if (DdU > 0)
                        {
                            intersects[1] = HugeNumber.PositiveInfinity;
                        }
                        else
                        {
                            intersects[1] = intersects[0];
                            intersects[0] = HugeNumber.PositiveInfinity;
                        }
                    }
            ...
            else if (!c1.IsZero)
            {
                ...
                if ((DdU * t) + DdPmV >= 0)
                {
                    ...
                    {
                        intersects[0] = HugeNumber.NegativeInfinity;
                        intersects[1] = t;
                    }
                }
                else
            ...
            {
                var invAD = 1 / DdU;
                var hInterval = new HugeNumber[2];
                if (DdU > 0)
                {
        ...
        {
            var angle = HugeNumber.Atan(Radius / Length);
            var sinAngle = HugeNumber.Sin(angle);
            var cosAngle = HugeNumber.Cos(angle);
