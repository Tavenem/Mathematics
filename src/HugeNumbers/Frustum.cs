...
namespace Tavenem.Mathematics.HugeNumbers
{
    ...
    public class Frustum : IShape, IEquatable<Frustum>, ISerializable
    {
        ...
        /// </summary>
        [DataMember(Order = 1)]
        public HugeNumber AspectRatio { get; }
        ...
        /// </summary>
        [DataMember(Order = 3)]
        public HugeNumber FieldOfViewAngle { get; }
        ...
        /// </summary>
        [System.Text.Json.Serialization.JsonIgnore]
        public HugeNumber FarPlaneDistance { get; }
        ...
        /// </summary>
        [DataMember(Order = 4)]
        public HugeNumber NearPlaneDistance { get; }
        ...
        /// </summary>
        public Frustum() : this(HugeNumber.Zero, Vector3.Zero, HugeNumber.Zero, HugeNumber.Zero) { }
        ...
        /// clipping plane occurs.
        /// </param>
        public Frustum(HugeNumber aspectRatio, Vector3 axis, HugeNumber fieldOfViewAngle, HugeNumber nearPlaneDistance)
        ...
        public Frustum(HugeNumber aspectRatio, Vector3 axis, HugeNumber fieldOfViewAngle, HugeNumber nearPlaneDistance, Vector3 position)
        ...
        public Frustum(HugeNumber aspectRatio, Vector3 axis, HugeNumber fieldOfViewAngle, HugeNumber nearPlaneDistance, Vector3 position, Quaternion rotation)
        {
            ...
            FarPlaneDistance = farPlaneDistSq.Sqrt();

            var tan = HugeNumber.Tan(fieldOfViewAngle);
            ...
            ContainingRadius = ((FarPlaneDistance - nearPlaneDistance).Square() + (tanSq4 * farPlaneDistSq * (1 + aspectRatio.Square()))).Sqrt() / 2;

            axis = position - (axis / 2);
            var basis1 = axis.Normalize().Transform(rotation);
            Vector3 basis2, basis3;
            if (basis1.Z.IsNearlyEqualTo(-1))
            {
        ...
        private Frustum(SerializationInfo info, StreamingContext context) : this(
            (HugeNumber?)info.GetValue(nameof(AspectRatio), typeof(HugeNumber)) ?? default,
            (Vector3?)info.GetValue(nameof(Axis), typeof(Vector3)) ?? default,
            (HugeNumber?)info.GetValue(nameof(FieldOfViewAngle), typeof(HugeNumber)) ?? default,
            (HugeNumber?)info.GetValue(nameof(NearPlaneDistance), typeof(HugeNumber)) ?? default,
        ...
        private static HugeNumber GetDistance(Frustum frustum, Vector3 cornerA, Vector3 cornerB, Vector3 cornerC, Vector3 cornerD)
        {
        ...
        private static HugeNumber GetEdgeDistance(Frustum frustum, Cuboid cuboid)
        {
            var distance = HugeNumber.PositiveInfinity;
        ...
        private static HugeNumber GetEdgeDistance(Frustum frustumA, Frustum frustumB)
        {
            var distance = HugeNumber.PositiveInfinity;
        ...
        private static HugeNumber GetFaceDistance(Plane[] planes, Vector3[] corners)
        {
            var distance = HugeNumber.NegativeInfinity;
            for (var i = 0; i < planes.Length; i++)
            {
                distance = HugeNumber.Max(distance, planes[i].DotCoordinate(GetSupport(corners, -planes[i].Normal)));
            }
            return distance;
        }
        ...
        {
            var maxProjection = HugeNumber.NegativeInfinity;
            var index = -1;
        ...
        public bool IsPointWithin(Vector3 point)
        {
            ...
            for (var i = 0; i < 6; i++)
            {
                if (Planes[i].DotCoordinate(point).Sign() != Planes[i].DotCoordinate(Position).Sign())
                {
                    return false;
                }
            }
        ...
        public IShape ScaleVolume(HugeNumber factor)
        {
            ...
            {
                var multiplier = factor.CubeRoot();
                return new Frustum(AspectRatio, Axis * multiplier, FieldOfViewAngle, NearPlaneDistance * multiplier, Position, Rotation);
            }
        }

        private HugeNumber GetPointDistance(Vector3 point)
        {
            var maxDistance = HugeNumber.NegativeInfinity;
            for (var i = 0; i < 6; i++)
            {
                var distance = Planes[i].DotCoordinate(point);
                distance = distance.Sign() == Planes[i].DotCoordinate(Position).Sign()
                    ? -HugeNumber.Abs(distance)
                    : HugeNumber.Abs(distance);
                maxDistance = HugeNumber.Max(maxDistance, distance);
            }
            return maxDistance;
        }
        ...
        private bool Intersects(Line line)
        {
            ...
            for (var i = 0; i < 6; i++)
            {
                ...
                var ratio = HugeNumber.Abs(distA) / HugeNumber.Abs(distB);
