...
public class CapsuleTests
{
    ...
    public void SerializationTest()
    {
        ...
        var shape = new Capsule<double>(axis, radius, position);
        IShape<double> iShape = shape;

        var json = JsonSerializer.Serialize(shape);
        Console.WriteLine();
        Console.WriteLine(json);
        Assert.AreEqual(shape, JsonSerializer.Deserialize<Capsule<double>>(json));
        ...
        json = JsonSerializer.Serialize(shape, MathematicsSourceGenerationContext.Default.CapsuleDouble);
        Console.WriteLine();
        Console.WriteLine(json);
        Assert.AreEqual(
            shape,
            JsonSerializer.Deserialize(json, MathematicsSourceGenerationContext.Default.CapsuleDouble));
        Assert.AreEqual(
            shape,
