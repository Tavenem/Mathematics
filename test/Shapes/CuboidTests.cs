...
public class CuboidTests
{
    ...
    public void SerializationTest()
    {
        ...
        var shape = new Cuboid<double>(axisX, axisY, axisZ, position, rotation);
        IShape<double> iShape = shape;

        var json = JsonSerializer.Serialize(shape);
        Console.WriteLine();
        Console.WriteLine(json);
        Assert.AreEqual(shape, JsonSerializer.Deserialize<Cuboid<double>>(json));
        ...
        json = JsonSerializer.Serialize(shape, MathematicsSourceGenerationContext.Default.CuboidDouble);
        Console.WriteLine();
        Console.WriteLine(json);
        Assert.AreEqual(
            shape,
            JsonSerializer.Deserialize(json, MathematicsSourceGenerationContext.Default.CuboidDouble));
        Assert.AreEqual(
            shape,
